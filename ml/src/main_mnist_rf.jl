...
model = RandomForestClassifier(n_estimators = 150, n_jobs = 7, criterion = "entropy")


num_features = size(tr_X, 2)

#r1 = range(model, :min_samples_split, lower = 2, upper = 12)
#st_boost = TunedModel(model = model, range = r1, tuning = Grid(resolution = 10), acceleration=CPU1(), acceleration_resampling=CPU1())
